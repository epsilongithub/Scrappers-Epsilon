<?php
	...
	const BLOQUE = '5';
	const TABLA_ICARUS_CONTENT = 'scrapper_ig_posts_contents';
	...
	*/
	...
	class InstagramScraper {
		...
		*/
		...
		function searchPost($linkPost, $tabla) {

	
        	$q = "SELECT * FROM ".$tabla." WHERE link='".$linkPost."'";
       ...
       function searchLikes($linkPost) {
		...
		function storePostUnique($id, $p, $url, $num){
       		...
       		$linkecito = $p->getLink();
			$banderita = $this->searchPost($linkecito, TABLA_ICARUS_CONTENT);
			$banderita2 = $this->searchPost($linkecito, TABLA_NOCTURNA_CONTENT);
			...
			if($banderita2 == true){
				if($num == 0){
					...
					$insertQuery2 = "UPDATE `".TABLA_NOCTURNA_CONTENT."` SET `likes` = ".$likesFinales.", `campo_7` = ".$p->getNumViews().", `actualizacion` = now() WHERE `link` = '".$linkecito."'";

				}else{

					$insertQuery2 = "UPDATE `".TABLA_NOCTURNA_CONTENT."` SET `likes` = ".$likesFinales.", `campo_7` = ".$p->getNumViews().", `comments` = ".$p->getNumComments().", `actualizacion` = now() WHERE `link` = '".$linkecito."'";
